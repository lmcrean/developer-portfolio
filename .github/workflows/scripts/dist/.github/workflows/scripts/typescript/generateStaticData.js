...
exports.StaticDataGenerator = void 0;
const dotenv_1 = __importDefault(require("dotenv"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const github_1 = require("../../../../apps/api/github/src/github");
const rest_1 = require("@octokit/rest");
const rateLimitUtils_1 = require("../../../../apps/api/github/src/utils/rateLimitUtils");
...
class StaticDataGenerator {
    constructor() {
        this.perPage = 20;
        this.enhanceExternalPRs = true;
        this.maxExternalPRsPerPage = 10;
        ...
        this.githubService = new github_1.GitHubService(githubToken);
        this.octokit = new rest_1.Octokit({ auth: githubToken });
        this.username = githubUsername;
        // Set up directory paths - point to API's static directory
        ...
        Object.entries(LIMITED_REPOSITORIES).forEach(([repoName, filterType]) => {
            if (filterType === 'keep-latest-only') {
                ...
                if (repoPRs.length > 1) {
                    ...
                    const mostRecentPR = repoPRs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0];
     ...
     */
    async enhanceExternalPRsWithDetailedData(prs) {
        ...
        for (const pr of prsToEnhance) {
            try {
                ...
                await (0, rateLimitUtils_1.delay)(150);
            }
            catch (error) {
     ...
     */
    async enhanceAllExternalPRs(externalPRs) {
        if (!this.enhanceExternalPRs) {
            return externalPRs;
        }
        ...
        for (const pr of prsNeedingEnhancement) {
            try {
                ...
                // Add delay between API calls to be respectful
                await (0, rateLimitUtils_1.delay)(200);
            }
            catch (error) {
                console.warn(`⚠️ Failed to enhance PR ${pr.number}:`, error);
                continue;
     ...
     */
    async generateExternalPRPageFile(page, allExternalPRs) {
        try {
     ...
     */
    async generateMetadataFile(totalCount, totalPages, pagesGenerated, enhancedPRsCount = 0) {
        const metadata = {
...
exports.StaticDataGenerator = StaticDataGenerator;
/**
